<header class="header-top">
    <div class="body-header-top">
        <div class="container d-flex align-items-center  justify-content-between">
            <div class="header-top-item">
                <a href="#">
                    <img class="img-item"
                         src="https://cdn2.cellphones.com.vn/x/https://dashboard.cellphones.com.vn/storage/207x30_TopBanner_Homepage_01.2024_Mb_2-01133.svg"
                         alt="" loading="lazy">
                </a>
            </div>
            <div class="header-top-item">
                <a href="#">
                    <img class="img-item"
                         src="https://cdn2.cellphones.com.vn/x/https://dashboard.cellphones.com.vn/storage/207x30_TopBanner_Homepage_01.2024_Mb_2-02133.svg"
                         alt="" loading="lazy">
                </a>
            </div>
            <div class="header-top-item">
                <a href="#">
                    <img class="img-item"
                         src="https://cdn2.cellphones.com.vn/x/https://dashboard.cellphones.com.vn/storage/207x30_TopBanner_Homepage_01.2024_Mb_2-03133.svg"
                         alt="" loading="lazy">
                </a>
            </div>
        </div>
    </div>
</header>
<header class="header-main d-flex align-items-center justify-content-center">
    <div class="text-center d-flex align-items-center  justify-content-between body-header-main">
        <a href="#" class="logo-item">
            <svg xmlns="http://www.w3.org/2000/svg" xmlns:xlink="http://www.w3.org/1999/xlink" viewBox="0 0 269.231 50">
                <defs>
                    <pattern id="patternLogo" preserveAspectRatio="xMidYMid slice" width="100%" height="100%"
                             viewBox="0 0 530 95">
                        <image width="530" height="95"
                               xlink:href="data:image/png;base64,"></image>
                    </pattern>
                </defs>
                <rect id="header__logo--bg" width="269.231" height="50" fill="url(#patternLogo)"></rect>
            </svg>
        </a>
        <a href="#" class="d-flex justify-content-between align-items-center navbar-category text-white ">
            <i class="fa-solid fa-bars-staggered pr-2"></i> <span>Danh mục</span>
        </a>

        <a href="#" class="d-flex justify-content-between align-items-center navbar-category text-white ">
            <i class="fa-solid fa-location-dot pr-2"></i>
            <span class="d-flex nav-wrap p-0">
               <span class="small">Xem giá tại</span>
               <span>Hồ Chí Minh</span>
           </span>
        </a>
        <div class="d-flex justify-content-between align-items-center navbar-input-search bg-white">
            <i class="fa-solid fa-magnifying-glass pl-3"></i>
            <input type="text" class="input-search" placeholder="Bạn cần tìm gì?">
        </div>
        <a href="#" class="d-flex justify-content-between align-items-center navbar-category text-white ">
            <i class="fa-solid fa-phone pr-2"></i>
            <span class="d-flex nav-wrap p-0">
               <span class="small">Gọi mua hàng</span>
               <strong class="small">1800.2097</strong>
           </span>
        </a>
        <a href="#" class="d-flex justify-content-between align-items-center navbar-category text-white ">
            <i class="fa-solid fa-location-dot pr-2"></i>
            <span class="d-flex nav-wrap p-0">
               <span class="small">Cửa hàng gần bạn</span>
           </span>
        </a>
        <a href="{{ route('order.me.list') }}" class="d-flex justify-content-between align-items-center navbar-category text-white ">
            <i class="fa-solid fa-truck"></i>
            <span class="d-flex nav-wrap p-0">
               <span class="small">Tra cứu đơn hàng</span>
           </span>
        </a>
        <a href="{{ route('cart.show') }}" class="d-flex justify-content-between align-items-center navbar-category text-white ">
            <i class="fa-solid fa-cart-shopping"></i>
            <span class="d-flex nav-wrap p-0">
               <span>Giỏ hàng</span>
           </span>
        </a>
        <a href="{{ Auth::check() ? route('user.profile.show') : route('auth.processLogin') }}" class="d-flex justify-content-between align-items-center nav-wrap navbar-category text-white ">
            <i class="fa-solid fa-user"></i>
           @if(Auth::check())
                <span>Profile</span>
           @else
                <span>Đăng nhập</span>
           @endif
        </a>
    </div>
</header>
